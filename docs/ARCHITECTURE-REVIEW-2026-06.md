...
| 8 | Recreate machinery | **Auditability via enumeration** (reshaped from "reuse the view walker", which was inaccurate — types are depended on by table columns, not view bodies, so the view walker doesn't apply, and the destruction is inherent). The plan now enumerates every dependent of a CASCADE-replaced type and names them in the destructive warning; the gated `CASCADE` teardown is kept. Full explicit type-dependent recreation deferred to post-1.0 ([v1.md §7](./v1.md)). | impl plan |
