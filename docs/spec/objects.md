...
| Object | Status | Notes |
|---|---|---|
| `TABLESPACE` | 📋 Planned, v0.4.2 | v0.4.2 adds `CREATE TABLESPACE` / `DROP TABLESPACE` management via the `pgevolve cluster …` surface (reversing the previous out-of-scope stance — see [`roadmap.md`](./roadmap.md) and the [skeleton plan](../superpowers/plans/_skeleton/cluster-tablespace.md)). Filesystem-layout management (directory creation, mount points) stays out of scope. The IR carries the `tablespace` attribute on tables and indexes today; per-partition overrides are tracked separately (📋 Planned, v0.4.0). |
