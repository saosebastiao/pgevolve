...
///
/// Each type goes through the same lowering the source-side `CREATE AGGREGATE`
/// parser uses, so the two `arg_types` lists compare equal.
fn parse_arg_types(
    arg_signature: &str,
    qname: &QualifiedName,
    location: &SourceLocation,
    ...
    let params = from_catalog::parameter_list(
        "pg_get_function_identity_arguments (aggregate)",
        arg_signature,
        location,
    )
    .map_err(|e| {
        CatalogError::Ir(crate::ir::IrError::InvalidIdentifier(format!(
            "catalog aggregate arg parse for {qname}: {e}"
        )))
    })?;
