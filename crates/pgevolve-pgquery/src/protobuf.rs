//! The Postgres parse tree, as prost-generated Rust.
//!
//! Generated by `prost-build` from `libpg_query/protobuf/pg_query.proto` and
//! **checked in as an ordinary source file**. Upstream regenerated it during
//! every build that had `protoc` on `PATH`, writing into its own `src/`
//! directory — which for a registry dependency means mutating the Cargo cache.
//! See `build.rs` for the reproduction. Regenerating this file is a deliberate
//! maintainer action, documented in `README.md`, not a build side effect.
//!
//! # Why the blanket allow
//!
//! Roughly 3,400 lints fire here, and none of them are actionable:
//!
//! - ~2,700 are `missing_docs` on the struct fields and enum variants of 272
//!   generated message types. The names come from Postgres's own parse nodes;
//!   documenting them here would mean paraphrasing the Postgres source into
//!   3,400 comments that then rot independently of it.
//! - ~220 are missing backticks in doc comments, because the generator carries
//!   the C comments over verbatim and they were never Rustdoc.
//! - The rest are style lints (`const fn`, `derive_partial_eq_without_eq`,
//!   `struct_excessive_bools`) about code no human wrote or will edit.
//!
//! Per CLAUDE.md §8 the allow is justified rather than casual, and it is scoped
//! to this file alone: every hand-written module in this crate is held to the
//! full workspace lint set. `prost` applies the same blanket allow to its own
...
#![allow(
    ...
    clippy::nursery,
    clippy::doc_markdown,
    reason = "prost-generated from pg_query.proto; see the module comment"
)]
