...
fn run(cli: Cli) -> Result<u8, anyhow::Error> {
    ...
    let code: i32 = runtime.block_on(async move {
        match cli.cmd {
            ...
            Command::Doctor { db, url } => {
                commands::doctor::run(&cfg, &db, url.as_deref()).await
            }
            Command::RewriteTable { qname, db, url: _, confirm_rewrite } => {
                commands::rewrite_table::run(&qname, &db, confirm_rewrite)
            }
            Command::Graph { graph_format, out, plan } => {
