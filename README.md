...
Current release: **v0.4.6** (Postgres 14–17; 18 partial — see below). See
...
> Postgres 18 — so PG 18-only DDL *parses*. `GENERATED ... VIRTUAL` and
> `NOT ENFORCED` constraints are fully supported: they round-trip through the
> IR, render, diff, and carry plan-time lints that block a plan targeting an
> older server. Still unmodelled are temporal `PERIOD` / `WITHOUT OVERLAPS`
> keys, which are **refused with an explicit error** on both the source and
> catalog sides rather than being silently mis-modelled as plain keys. Refusing is
...
- **All actively-maintained PG majors.** PG 14, 15, 16, 17, 18 covered in
  ...
  syntax parses, and the conformance suite runs all 263 fixtures against every
  major. One PG 18 feature is not modelled yet and is refused rather than
  approximated — see the note above.
- **Conformance-driven.** ~263 fixture-based end-to-end tests gate
